...
def C_matrix(f, z, normalize=False):
    '''
    f:      model.decode: (b, d) -> (b, n) 
    z:      torch.tensor whose size = (b, d) (keep b low for memory)
...
def get_Riemannian_metric(f, z, create_graph=False):
    '''
    f:      model.decode: function (b, d) -> (b, n)
    z:      torch.tensor whose size = (b, d)
    out:    torch.tensor whose size = (b, n, n)
    Outputs the Riemannian metric on the latent space 
    '''
    J = jac_robust(f, z, create_graph=create_graph)
    out = torch.einsum('nij,nik->njk', J, J)  #J.T @ J instead! 
    return out

# from https://github.com/Gabe-YHLee/GM4HDDA
def compute_length_of_curve(curve, f, get_Riemannian_metric):
    '''
    ...
    G = get_Riemannian_metric(f, curve[:-1]) # (L-1, d, d)
...
method="powell", options={"maxiter":1000, "disp":True}, loss_scale=10000, tol=1e-8, verb=False, dev=f"cuda:{0}"):
    ...
    class GeodesicFittingTool():
        ...
        def jac(self, z):
            ...
            z_extended = torch.cat([self.z_init.unsqueeze(0), z_torch, self.z_final.unsqueeze(0)], dim=0)
            G_ = self.get_Riemannian_metric(self.pretrained_model, z_extended[:-1], create_graph=True)
            delta_z = (z_extended[1:, :]-z_extended[:-1, :])/(self.delta_t)
...
def compute_linear(z1, z2, num_discretization=100, dev=f"cuda:{0}"):
    '''
    z1 : torch.tensor whose size = (1, 2)
    z1 : torch.tensor whose size = (1, 2)
    out: torch.tensor whose size = (num_discretization, 2)
    Outputs the geodesic curve between z1 and z2
    '''
    lin_curve = torch.cat([z1.to(dev) + (z2.to(dev) - z1.to(dev)) * t/(num_discretization-1) for t in range(num_discretization)], dim=0)
    return lin_curve